...
## 18/09/2026, à tarde — a montra «Promos» dentro da Coleção (`promos.py`, `data/promos_oficiais.json`)

Palavras dele: *"e essas que digo, de Nexus Night, eventos, bundles, etc"* /
*"consegues averiguar e fazer um botao com essas, **com foto da carta
tambem**, assim consigo perceber se vou querer coleccionar tambem ou nao"*.
Ramo `ai-pc/promos-2026-09-18`; relatório em
`ai-pc/work/revisao/riftvault-promos.md`.

**É UMA MONTRA, NÃO UMA META.** O objectivo dele é VER o que existe para
decidir se vai atrás disto. Não conta para nada — nem percentagem, nem
denominador (928), nem wantlist, nem «A mais», nem Faltas, nem valor —, não
tem alvo, não entra em lista de compra nenhuma, e **não é uma categoria nova**
ao lado de Alt Art / OverNumbered / SP (o `master_set.*` não mudou). Provado:
`tests/test_promos_montra.py::TestNadaMexe` (antes/depois iguais; nenhum
módulo das contas importa o `promos`) e a medição contra cópias do `data/`
real (`_revisao\_medir_promos.py`, `main` `c59a471` vs ramo, a montra pedida
ANTES de medir o lado «depois»): denominador **928**, níveis **860/780/715 =
92,7 / 84,1 / 77,0 %**, wantlist «tudo» **213 · 406 · 1 416,51 €**, valor
**3 005,89 € · 2 391 cópias**, falta dos decks **9 · 6 · 231,93 €**, compras,
Encomendas (4 cópias · 164,22 €), Faltas por edição (390 · 732 · 13 342,62 €),
A mais (65 · 131 / 37 · 68, item a item), os separadores (310/24/251/238/203
grupos) e os blocos da grelha — **iguais** nos dois lados.

**A lista é DADOS, versionada, e ele corrige-a à mão.** `data/promos_oficiais.json`
(`config.PROMOS_PATH`, `RIFTVAULT_PROMOS` para os testes; presa à raiz do
repo e não ao `DATA_DIR`, como a pasta `decks/`): 143 entradas, 128 cartas
distintas, `{categoria, origem, nota, nome}`, com a fonte
(riftbound.gg — «Riftbound Promo Cards Complete List & How to Get?»), o URL,
o `lido_em` (2026-09-18) e o aviso de que é uma lista de comunidade lida UMA
vez à mão. **NÃO SE VOLTA AO `riftbound.gg`** — o `robots.txt` deles proíbe
o `anthropic-ai`; nada no código faz rede, nem nenhuma tarefa.

**O catálogo NÃO conhece estas promos.** A RiftScribe só tem 6 promos
(`ven-sp1..sp6`); as 128 cartas existem lá como cartas NORMAIS. Por isso a
foto é a da **versão normal** (`promos._referencia`: a base da edição mais
antiga, nem alt art nem sobrenumerada) e a página, o tile e o CLI dizem-no
(`promos.AVISO_FOTO`).

**O casamento dos nomes NUNCA adivinha** (`promos.casar`), quatro passos, cada
um só vale quando dá UMA carta: `exacto` (o `card_key`); `mercado` (o
`cardtrader_map.market_name` — «Lux - Crownguard» é a «Lux, Crownguard»,
«Master Yi - Honed» é a «Yi, Honed»: é o nome REGISTADO da mesma impressão
noutra fonte, não uma aproximação); `sem separador` (a vírgula/hífen do
subtítulo: «Riven Shattered» = «Riven, Shattered»); `cauda` («Darius, Hand of
Noxus» → «Hand of Noxus», **só se a carta for um Legend** — a regra do
`decks.resolve`). No catálogo real: **125 de 128** (80 exacto, 38 mercado,
1 sem separador, 6 cauda); **3 por encontrar**, na lista «Não encontradas no
catálogo» da página: `K'Sante Courageous` (não existe na RiftScribe), `Bird //
Brush` e `Recruit//Buff` (cartas de dupla face do Vault/TFT; as metades
existem como tokens, mas casar uma entrada com duas cartas era inventar).

**Ter a carta NÃO é ter o promo.** O `vault.db` conta cópias por impressão do
catálogo, e o promo não é uma impressão do catálogo — não há onde registar
que ele tem a versão promo. Cada item leva `tens` (todas as impressões da
carta, `metrics.owned_by_card`, sem as retiradas) e `tens_por` (por
impressão), e a página diz a ressalva (`promos.AVISO_TER`). **Não se inventou
um «tens 1/1».** Quando a carta também tem uma `VEN-SP` no catálogo (hoje só
a Lux, Crownguard → `VEN-SP6`), o item leva `promo_catalogo` e o tile diz «a
mesma carta, vista do VEN» — as 6 `VEN-SP` continuam onde estão, com o alvo
que têm.

**Onde vive.** `riftvault/promos.py` (`carregar`, `casar`, `payload`; rebenta
com `ListaInvalida` se o ficheiro faltar ou não tiver `categoria`+`nome`);
rota `GET /api/promos.json` (404 com a razão se a lista não prestar); o
`build` escreve `api/promos.json`; `riftvault promos [--categoria X]
[--so-tenho]`. No site: o botão **Promos** no fim do `#set-tabs` da Coleção
(`renderSetTabs`), `loadPromos`/`renderPromos`/`prTile` no `app.js`, o
`#promos` dentro de `<section id="colecao">`; enquanto está aberta o resto da
Coleção fica escondido pelo CSS `#colecao.promos-on` (o `state.setId`/`payload`
não se tocam; o teclado da grelha fica desligado por `state.promosOn`);
`prefs.set = 'promos'` sobrevive ao refresh, e `#promos` no URL abre-a. Os
tiles são os `dtile` das Faltas (a cores — não falta nada), com a origem no
canto (`.pr-origem`), «foto: versão normal» em cima da foto, a nota do evento,
«tens N da carta (3× SFD-013)» e «também: Bundle (Worlds 2025)» quando o mesmo
nome está noutra categoria. As categorias vêm por tamanho (Nexus Night 83,
Bundle 17, Evento de lançamento 14, Summoner Skirmish 10, e os pequenos); dentro
de cada uma é a ordem do ficheiro. Um `+`/`−` na grelha ou um «Chegou» põem
`state.promos` a `null` para o «tens N» ser relido.

`tests/test_promos_montra.py` (25 testes, contra cópias, config temporário e
uma lista de brincar em `RIFTVAULT_PROMOS` — nunca a lista real, que ele vai
corrigir). Suite: 33 ficheiros.
